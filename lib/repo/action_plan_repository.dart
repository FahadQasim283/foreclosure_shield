  ...
  Future<ApiResponse<ActionPlanResponse>> getActionPlan({String? assessmentId}) async {
    try {
      ...
      final errorMsg =
          ...
          e.message ??
          'Unknown error';
      debugPrint('Get action plan error: $errorMsg');
      return ApiResponse.failure(
        e.response?.data?['message'] ??
            e.response?.data?['error']?['message'] ??
            e.message ??
            'Failed to fetch action plan',
        error: ApiError(
          message:
              e.response?.data?['message'] ??
              e.response?.data?['error']?['message'] ??
  ...
  Future<ApiResponse<TaskDetailsResponse>> getTaskDetails(String taskId) async {
    try {
      ...
      final errorMsg =
          ...
          e.message ??
          'Unknown error';
      debugPrint('Get task details error: $errorMsg');
      return ApiResponse.failure(
        e.response?.data?['message'] ??
            e.response?.data?['error']?['message'] ??
            e.message ??
  ...
  Future<ApiResponse<UpdateTaskStatusResponse>> updateTaskStatus({
    ...
    try {
      ...
      final errorMsg =
          ...
          e.message ??
          'Unknown error';
      debugPrint('Update task status error: $errorMsg');
      return ApiResponse.failure(
        e.response?.data?['message'] ??
            e.response?.data?['error']?['message'] ??
            e.message ??
  ...
  Future<ApiResponse<UpdateTaskNotesResponse>> updateTaskNotes({
    ...
    try {
      ...
      final errorMsg =
          ...
          e.message ??
          'Unknown error';
      debugPrint('Update task notes error: $errorMsg');
      return ApiResponse.failure(
        e.response?.data?['message'] ??
            e.response?.data?['error']?['message'] ??
            e.message ??
  ...
  Future<ApiResponse<ActionPlanResponse>> getAllTasks() async {
    try {
      ...
      final errorMsg =
          ...
          e.message ??
          'Unknown error';
      debugPrint('Get all tasks error: $errorMsg');
      return ApiResponse.failure(
        e.response?.data?['message'] ??
            e.response?.data?['error']?['message'] ??
            e.message ??
